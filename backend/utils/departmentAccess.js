...
const canAccessCollegeByCompany = ({ user, college }) => {
    ...
    if (role === 'collegeadmin') {
        ...
        const scopeMatched = Array.isArray(user?.departmentAccess)
            && user.departmentAccess.some((scope) => scopeMatchesCollege(scope, college));
    ...
    const companyScopedRoles = new Set(['companyadmin', 'company', 'accountnt', 'spocadmin']);
...
const resolveVisibleDepartments = ({ user, college, requestedDepartment, salaryDepartments = [] }) => {
    ...
    const scopes = Array.isArray(user?.departmentAccess)
        ? user.departmentAccess.filter((scope) => scopeMatchesCollege(scope, college))
        : [];
