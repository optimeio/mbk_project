...
try {
  if (smsDisabled) {
    console.log("SMS mock mode enabled. Plivo client will not be initialized.");
  } else if (authId && authToken) {
    client = new plivo.Client(authId, authToken);
  } else {
    console.log("Plivo credentials missing. SMS mock mode enabled.");
  }
} catch (error) {
