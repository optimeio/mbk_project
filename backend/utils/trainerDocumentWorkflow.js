...
const buildTrainerDocumentMap = (trainerDocuments = []) =>
  trainerDocuments.reduce((accumulator, document) => {
    if (document?.documentType) {
      accumulator[document.documentType] = document;
    }
    return accumulator;
...
const evaluateTrainerDocumentWorkflow = (trainer = {}, trainerDocuments = []) => {
  ...
  const trainerDocumentMap = buildTrainerDocumentMap(trainerDocuments);
  ...
  const checklist = REQUIRED_TRAINER_DOCUMENTS.map(({ key, label }) => {
    const trainerDocument = trainerDocumentMap[key] || null;
    const uploaded = Boolean(
      docs[key] ||
        trainerDocument?.filePath ||
        trainerDocument?.driveViewLink ||
        trainerDocument?.driveDownloadLink,
    );
  ...
  return {
    ...
    canProceedToAgreement:
      hasAllRequiredDocuments && !hasRejectedDocuments,
  };
};
...
const syncTrainerDocumentWorkflow = (trainer, trainerDocuments = []) => {
  ...
  if (trainer) {
    ...
    trainerDocuments.forEach((document) => {
      ...
      if (documentType && storedPath && !trainer.documents[documentType]) {
        trainer.documents[documentType] = storedPath;
      }
    });
