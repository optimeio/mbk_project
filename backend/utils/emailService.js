...
const companyActivatedEmail = require("./companyActivatedEmail");
const {
  canUseGmailApi,
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        if (json.message) {
          parsedErr = json.message;
        }
      } catch {}
      throw new Error(parsedErr || `Resend API failed with status ${response.status}`);
    }
    let parsed = {};
    try {
      parsed = JSON.parse(body);
...
if (!smtpPass || smtpPass.length === 0) {
  ...
  console.log("📧 Email service configured (delivery checked on first send).");
}
...
const sendComplaintStatusUpdateEmail = async (
  ...
  const senderIdentity = `"MBK BY TSMG" <${smtpUser}>`; // Dynamic sender
...
const sendRegistrationOTP = async (userEmail, userName, otp) => {
  ...
  if (allowOtpDebug) {
    console.log(`[AUTH-OTP] OTP generated for ${userEmail} (debug mode enabled)`);
    try {
      const fs = require("fs");
      const path = require("path");
      const serverDebugFile = path.join(__dirname, "../otp_debug.txt");
      const rootDebugFile = path.join(__dirname, "../../otp_debug.txt");
      const logContent = `[${new Date().toISOString()}] Email: ${userEmail} | OTP: ${otp}\n`;
...
const sendCompanyApprovalEmail = async (adminEmail, companyName, loginUrl) => {
  const mailOptions = {
    from: process.env.EMAIL_FROM || `"MBK CarrierZ" <${smtpUser}>`,
    to: adminEmail,
    subject: `Your MBK CarrierZ Company Account Has Been Approved!`,
    html: companyActivatedEmail({
      companyName: companyName || "Company",
      adminName: companyName || "Company Admin",
  ...
  try {
    ...
    console.log("Company approval email sent successfully to:", adminEmail);
    return { success: true, messageId: result.messageId };
  } catch (error) {
