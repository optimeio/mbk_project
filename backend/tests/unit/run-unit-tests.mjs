...
const backendRoot = path.resolve(path.dirname(require.resolve("../../package.json")));
const {
  ...
  },
  {
    ...
    run: async () => {
      ...
      const payload = await listScheduleAssociationsFeed({
        ...
        listTrainers: async () => [
          { _id: "TRN-1", name: "Trainer One", trainerId: "T001" },
        ],
      ...
      assert.deepEqual(payload.data.companies, [
        { id: "COMP-1", _id: "COMP-1", name: "Company One" },
      ]);
      assert.deepEqual(payload.data.courses, [
        { id: "COURSE-1", _id: "COURSE-1", name: "Course One", title: "Course One", companyId: "COMP-1", colleges: [] },
      ]);
      assert.deepEqual(payload.data.colleges, [
        { id: "COL-1", _id: "COL-1", name: "College One", companyId: "COMP-1", courseId: "COURSE-1", department: "CSE, IT" },
      ]);
      assert.deepEqual(payload.data.trainers, [
        { id: "TRN-1", _id: "TRN-1", name: "Trainer One", trainerId: "T001", companyId: null, companyCode: null },
      ]);
      assert.deepEqual(payload.data.departments, [
        { id: "DEP-1", _id: "DEP-1", name: "CSE", companyId: "COMP-1", courseId: "COURSE-1", collegeId: "COL-1" },
        { id: "DEP-2", _id: "DEP-2", name: "IT", companyId: "COMP-1", courseId: "COURSE-1", collegeId: "COL-1" },
      ]);
