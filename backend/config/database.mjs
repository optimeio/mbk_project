...
const connectToDatabase = async () => {
  ...
  try {
    ...
    // If DB connection fails, enable background reconnect so transient connection errors don't crash nodemon
    const allowOffline = process.env.ALLOW_OFFLINE !== '0';
    if (allowOffline) {
      console.warn('⚠️ Starting background reconnect for MongoDB (server will remain active)...');
      startBackgroundReconnect(mongoUri, connectOptions);
