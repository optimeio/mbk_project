...
const resolveAllowedTypeForField = (fieldName = '') => {
    if (
        UNIFIED_ATTENDANCE_FIELDS.has(fieldName) ||
        ATTENDANCE_EXCEL_FIELDS.has(fieldName) ||
        ATTENDANCE_PDF_FIELDS.has(fieldName)
    ) {
        return {
            label: 'Attendance Document (Excel/PDF/Image)',
            allowedExtensions: ALL_ATTENDANCE_EXTENSIONS,
            allowedMimeTypes: ALL_ATTENDANCE_MIME_TYPES
        };
    }
