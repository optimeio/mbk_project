...
// Fallback for missing uploads under /api/uploads or /uploads
app.use(['/api/uploads', '/uploads'], async (req, res) => {
  res.setHeader('Cross-Origin-Resource-Policy', 'cross-origin');
  ...
  if (filename) {
    try {
      ...
      // 1. Check ScheduleDocument (stores direct Google Drive links for attendance/geotag/activities)
      try {
        const ScheduleDocument = (await import('./models/ScheduleDocument.js')).default;
        const schedDoc = await ScheduleDocument.findOne({
          $or: [
            { fileName: regex },
            { fileUrl: regex },
      ...
      const record = await Attendance.findOne({
        $or: [
          { 'checkOut.photos.url': regex },
          { checkOutGeoImageUrl: regex },
          { checkOutGeoImageUrls: regex },
          { checkInPhoto: regex },
          { checkInImage: regex },
          { imageUrl: regex },
          { 'checkIn.photo': regex },
      ...
      }).select('checkIn checkOut checkInPhoto checkInGeoImageUrl checkInImage imageUrl signatureUrl attendancePdfUrl attendanceExcelUrl driveFileId scheduleId').lean();
      ...
      if (record) {
        const driveId =
          ...
          extractDriveFileId(record?.imageUrl) ||
          extractDriveFileId(record?.signatureUrl) ||
        ...
        // Try checking ScheduleDocument linked to this attendance
        try {
          const ScheduleDocument = (await import('./models/ScheduleDocument.js')).default;
          const linkedSchedDoc = await ScheduleDocument.findOne({
            $or: [
...
const startServer = async (port = Number(process.env.PORT || process.env.BACKEND_PORT) || 5005) => {
  try {
    ...
    server.listen(port, () => {
      ...
      try {
        const { ensureSmGroupsCompanyForTrainers } = require("./scripts/assign-sm-groups-trainers.js");
        ensureSmGroupsCompanyForTrainers().catch((err) => {
          console.warn("[MIGRATION] SM Groups auto-assignment warning:", err.message);
        });
      } catch (smErr) {
        console.warn("[MIGRATION] Could not load SM Groups helper script:", smErr.message);
      }
