...
userSchema.pre('save', async function (next) {
  ...
  if (this.isModified('email') || this.isModified('phoneNumber') || this.isModified('profile.phone') || this.isNew) {
    try {
      const isCompanyAdmin = this.role && String(this.role).toLowerCase().includes('companyadmin');
      const companyId = this.companyId || null;
      // Validate email uniqueness across all collections
      if (this.isModified('email') || this.isNew) {
        await assertUniqueEmail(this.email, this._id, 'User', { companyId, isCompanyAdmin });
        this.email = this.email.toLowerCase().trim();
      }
      
      // Validate phoneNumber uniqueness (legacy field)
      if ((this.isModified('phoneNumber') || this.isNew) && this.phoneNumber) {
        await assertUniquePhone(this.phoneNumber, this._id, 'User', { companyId, isCompanyAdmin });
      }
      
      // Validate profile.phone uniqueness
      if ((this.isModified('profile.phone') || this.isNew) && this.profile?.phone) {
        await assertUniquePhone(this.profile.phone, this._id, 'User', { companyId, isCompanyAdmin });
      }
    } catch (error) {
      return next(error);
    }
