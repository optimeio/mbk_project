...
const notificationSchema = new mongoose.Schema({
    ...
    role: {
        type: String,
        required: true,
        enum: ['SuperAdmin', 'CompanyAdmin', 'CollegeAdmin', 'SPOCAdmin', 'Trainer', 'Accountnt', 'Student']
    },
    title: {
        type: String,
        required: true,
...
notificationSchema.index({ userId: 1, isRead: 1, createdAt: -1 }); // unread-first pagination
notificationSchema.index({ createdAt: -1 });
notificationSchema.index({ userId: 1, createdAt: -1 });
// TTL: auto-delete notifications older than 30 days to prevent collection bloat at scale
