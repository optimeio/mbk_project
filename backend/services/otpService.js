...
let client = null;
const smsDisabled = String(process.env.DISABLE_SMS || '').trim() === '1';
if (smsDisabled) {
    console.log('SMS mock mode enabled. OTP codes will be logged locally.');
} else if (process.env.PLIVO_AUTH_ID && process.env.PLIVO_AUTH_TOKEN) {
    client = new plivo.Client(process.env.PLIVO_AUTH_ID, process.env.PLIVO_AUTH_TOKEN);
    console.log('Plivo SMS service initialized');
} else {
    console.log('Plivo credentials missing. OTP SMS mock mode enabled.');
}
