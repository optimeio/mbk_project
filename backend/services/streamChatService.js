...
const isStreamChatDisabled = String(process.env.DISABLE_STREAM_CHAT || '').trim() === '1';
...
const isStreamChatConfigured = () => !isStreamChatDisabled && Boolean(apiKey && apiSecret);
...
if (!isStreamChatDisabled && (!apiKey || !apiSecret)) {
...
const UNSUPPORTED_CHAT_PORTAL_ROLES = new Set(['student', 'company', 'companyadmin']);

const isUnsupportedChatPortalRole = (role = '') =>
    UNSUPPORTED_CHAT_PORTAL_ROLES.has(normalizeRoleToken(role));
...
const createWorkspaceBootstrap = async (user) => {
    ...
    try {
        ...
        if (isUnsupportedChatPortalRole(role)) {
            return buildChatDisabledPayload(
                user,
                'Chat is not available for your account type.',
            );
        }
