...
const MAX_SENDS_PER_HOUR = Number(process.env.MAX_OTP_SENDS_PER_HOUR || 15);
...
const assertRateLimit = async (record) => {
  ...
  if (withinHour && requestCount >= MAX_SENDS_PER_HOUR) {
    const remainingSec = Math.ceil((60 * 60 * 1000 - (now - hourWindowStart)) / 1000 / 60);
    const err = new Error(
      `Too many OTP requests. Please wait ${remainingSec > 0 ? remainingSec + ' minutes' : 'a moment'} before requesting another code.`,
    );
    err.statusCode = 429;
    throw err;
  }
...
const sendEmailOtp = async ({
  ...
  if (allowDebugMode) {
    ...
    if (!sendResult?.success) {
      let rawErr = sendResult?.hint || sendResult?.error || "Unable to send verification email.";
      try {
        const json = JSON.parse(rawErr);
        if (json.message) rawErr = json.message;
      } catch {}

      console.warn("[AUTH-OTP] Email delivery failed or restricted by provider. Falling back to debug OTP for verification:", rawErr);
      deliveryMode = "debug";
      debugOtp = rawOtp;
    } else {
      console.log(
