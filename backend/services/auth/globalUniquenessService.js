 ...
 */
const checkEmailExists = async (email, excludeId = null, excludeModel = null, options = {}) => {
  ...
  try {
    ...
    const userExists = await getUser().findOne(userQuery).select('_id role companyId email').lean();
    if (userExists) {
      const isLinkedCompanyAdmin =
        excludeModel === 'Company' &&
        (String(userExists.role || '').toLowerCase().includes('companyadmin') ||
         (excludeId && String(userExists.companyId) === String(excludeId)) ||
         (options.userId && String(userExists._id) === String(options.userId)));

      if (!isLinkedCompanyAdmin) {
        return {
          exists: true,
          foundIn: 'User',
    ...
    const companyExists = await getCompany().findOne(buildQuery('Company')).select('_id adminId userId email').lean();
    if (companyExists) {
      const isCompanyForThisUser =
        excludeModel === 'User' &&
        (options.isCompanyAdmin ||
         (options.companyId && String(companyExists._id) === String(options.companyId)) ||
         (excludeId && (String(companyExists.adminId) === String(excludeId) || String(companyExists.userId) === String(excludeId))));

      if (!isCompanyForThisUser) {
        return {
          exists: true,
          foundIn: 'Company',
          role: 'Company',
 ...
 * @returns {Promise<{exists: boolean, foundIn: string|null, message: string}>}
 */
const checkPhoneExists = async (phone, excludeId = null, excludeModel = null, options = {}) => {
  ...
  try {
    ...
    const userExists = await getUser().findOne(buildQuery('User')).select('_id role companyId').lean();
    if (userExists) {
      const isLinkedCompanyAdmin =
        excludeModel === 'Company' &&
        (String(userExists.role || '').toLowerCase().includes('companyadmin') ||
    ...
    const companyExists = await getCompany().findOne(buildQuery('Company')).select('_id adminId userId').lean();
    if (companyExists) {
      const isCompanyForThisUser =
        excludeModel === 'User' &&
        (options.isCompanyAdmin ||
 ...
 */
const assertUniqueEmail = async (email, excludeId = null, excludeModel = null, options = {}) => {
  const result = await checkEmailExists(email, excludeId, excludeModel, options);
 ...
 */
const assertUniquePhone = async (phone, excludeId = null, excludeModel = null, options = {}) => {
  // Allow empty/null phone numbers (optional field)
  if (!phone) {
    return null;
  }
  
  const result = await checkPhoneExists(phone, excludeId, excludeModel, options);
 ...
 */
const validateUniqueness = async ({ email, phone, excludeId, excludeModel, ...restOptions }) => {
  const normalizedEmail = await assertUniqueEmail(email, excludeId, excludeModel, restOptions);
  const normalizedPhone = await assertUniquePhone(phone, excludeId, excludeModel, restOptions);
