...
const uploadToDriveWithRetry = async (
  ...
  console.warn(`[GOOGLE-DRIVE] All ${attempts} attempts failed. Falling back to local storage. Error: ${lastError.message}`);
  try {
    const fallbackDir = path.resolve(process.cwd(), "uploads", "drive_fallback");
    if (!fs.existsSync(fallbackDir)) {
      fs.mkdirSync(fallbackDir, { recursive: true });
    }
    const safeFileName = sanitizeFileName(uploadPayload.fileName || uploadPayload.originalName || `fallback-${Date.now()}`);
    const localFileName = `${Date.now()}-${safeFileName}`;
    const localFilePath = path.join(fallbackDir, localFileName);
    
    await fs.promises.writeFile(localFilePath, uploadPayload.fileBuffer);
    
    // Using dummy fileId prefix "local-" to distinguish
    const dummyFileId = `local-${localFileName}`;
    const localUrl = `/api/uploads/drive_fallback/${localFileName}`;
    
    console.log(`[GOOGLE-DRIVE-FALLBACK] Successfully saved to local fallback: ${localFilePath}`);
    
    return {
      fileId: dummyFileId,
      folderId: uploadPayload.folderId || "local",
    ...
    console.error(`[GOOGLE-DRIVE-FALLBACK] Local fallback also failed: ${fallbackError.message}`);
    throw lastError; // Throw original drive error if fallback fails
  }
};
...
const deleteFromDrive = async (fileId) => {
  ...
  if (typeof fileId === "string" && fileId.startsWith("local-")) {
    try {
      ...
      console.warn("[GOOGLE-DRIVE-FALLBACK] Failed to delete local fallback file:", e.message);
    }
    return;
  }
