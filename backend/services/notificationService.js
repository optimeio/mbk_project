...
const isTwilioAccountSidValid = (sid) =>
  typeof sid === "string" && /^AC[a-zA-Z0-9]{32}$/.test(sid);

try {
  const twilioAccountSid = process.env.TWILIO_ACCOUNT_SID?.trim();
  const twilioAuthToken = process.env.TWILIO_AUTH_TOKEN?.trim();

  if (isTwilioAccountSidValid(twilioAccountSid) && twilioAuthToken) {
    client = twilio(twilioAccountSid, twilioAuthToken);
  } else if (twilioAccountSid || twilioAuthToken) {
    logNotificationTelemetry("warn", {
      stage: "twilio_client_unconfigured",
      status: "notification_setup",
      outcome: "skipped",
      reason: "Twilio credentials are present but invalid or incomplete",
    });
  }
} catch (error) {
  logNotificationTelemetry("warn", {
