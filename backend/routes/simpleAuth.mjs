...
const {
  ...
  verifyEmailOtp,
  OTP_PURPOSE,
} = require('../services/auth/authEmailOtpService.js');
...
router.post('/company/send-otp', async (req, res) => {
  try {
    const { email } = req.body;
    if (!email) {
    ...
    const result = await sendEmailOtp({
      email,
      purpose: OTP_PURPOSE.COMPANY_ADMIN_VERIFY || 'company_admin_verify',
    ...
    return res.json({
      success: true,
      message: result?.debugOtp ? 'Verification code generated' : 'OTP sent to email for verification',
      debugOtp: result?.debugOtp || undefined,
    });
  } catch (err) {
    console.error('Send company OTP error:', err);
...
router.post('/company/register', async (req, res) => {
  try {
    const { companyName, adminName, email, phone, address, website, password, confirmPassword, otp } = req.body;
    ...
    // Create corresponding User account pending approval
    ...
    newCompany.userId = newUser._id;
    await newCompany.save();

    return res.status(201).json({
      success: true,
      pendingApproval: true,
      message: 'Company registration successful! Your account is pending admin approval. You will receive an email once approved.',
      company: {
        id: newCompany._id,
...
router.post('/company/login', async (req, res) => {
  try {
    ...
    const companyStatus = String(company.status || 'pending').toLowerCase();
    const userStatus = String(user?.accountStatus || companyStatus).toLowerCase();

    if (companyStatus === 'pending' || userStatus === 'pending') {
      return res.status(403).json({
        success: false,
        pendingApproval: true,
        message: 'Your company account is pending admin approval. Please wait for admin approval.',
      });
    }

    if (companyStatus === 'inactive' || companyStatus === 'rejected' || userStatus === 'rejected') {
      return res.status(403).json({
        success: false,
        message: 'Your company account is inactive or has been rejected. Please contact MBK support.',
      });
    }
