...
const isSPOCAdmin = (req, res, next) => {
    ...
    const allowedRoles = new Set(['spocadmin', 'superadmin', 'admin', 'companyadmin', 'company', 'accountnt', 'accountant', 'collegeadmin', 'trainer']);
...
const buildDayUploadStatus = (schedule, attendance) => {
    ...
    const hasCheckIn = Boolean(attendance?.checkIn?.time || attendance?.checkInTime);
...
const ensureDepartmentsAndSchedules = async (college, preferredDepartmentName = '') => {
    ...
    // We no longer auto-generate 12 dummy empty schedules per department.
    // Explicit schedules must be created via the Add Schedule API.
...
router.get('/:id', authenticate, async (req, res) => {
    try {
        ...
        if (normalizeRole(req.user?.role) === 'trainer') {
            ...
            const isAssignedDirectly = trainerProfile && assignedTrainerIds.includes(String(trainerProfile._id));
            if (!isAssignedDirectly) {
                const { Schedule, TrainerAssignment } = require('../models');
                const candidateIds = [req.user.id, trainerProfile?._id].filter(Boolean);
                const hasSchedule = await Schedule.exists({
                    collegeId: college._id,
                    trainerId: { $in: candidateIds }
                });
                const hasAssignment = await TrainerAssignment.exists({
                    collegeId: college._id,
                    trainerId: { $in: candidateIds }
                });
                if (!hasSchedule && !hasAssignment) {
                    return res.status(403).json({ message: 'Access denied for this college' });
                }
            }
        }
...
router.get('/:id/details', authenticate, async (req, res) => {
    try {
        const { Schedule, Attendance, TrainerAssignment } = require('../models');
        ...
        collegeTrainers.forEach((t) => {
            if (t.userId?.isActive !== false) {
                trainerMap.set(String(t._id), t);
            }
        });
        allActiveTrainers.forEach((t) => {
            if (t.userId?.isActive !== false && !trainerMap.has(String(t._id))) {
                trainerMap.set(String(t._id), t);
            }
        });
        const trainers = Array.from(trainerMap.values()).sort((a, b) => {
            const nameA = a.userId?.name || a.name || '';
            const nameB = b.userId?.name || b.name || '';
        ...
        const days = schedulesWithAttendance.map(({ schedule, attendance }) => {
            ...
            const resolvedTrainerName = schedule.trainerId?.userId?.name || attendance?.trainerId?.userId?.name || (schedule.trainerId ? 'Trainer' : null);

            const checkOutImages = Array.isArray(attendance?.checkOutGeoImageUrls) && attendance.checkOutGeoImageUrls.length
                ? attendance.checkOutGeoImageUrls
                : (attendance?.checkOutGeoImageUrl ? [attendance.checkOutGeoImageUrl] : (Array.isArray(attendance?.checkOut?.images) ? attendance.checkOut.images.map(i => i?.image).filter(Boolean) : (Array.isArray(attendance?.checkOut?.photos) ? attendance.checkOut.photos.map(p => p?.url).filter(Boolean) : [])));

            return {
                id: schedule._id,
                dayNumber: schedule.dayNumber,
                session: schedule.session || 'FULL_DAY',
                ...
                attendanceImage: attendance?.studentsPhotoUrl || attendance?.imageUrl || null,
                attendancePdfUrl: attendance?.attendancePdfUrl || attendance?.scannedAttendancePdfUrl || null,
                attendanceExcelUrl: attendance?.attendanceExcelUrl || null,
                checkInImage: attendance?.checkInImage || attendance?.checkInPhoto || null,
                checkInPhoto: attendance?.checkInPhoto || attendance?.checkInImage || null,
                checkOutGeoImageUrl: attendance?.checkOutGeoImageUrl || attendance?.checkOutImage || null,
                checkOutGeoImageUrls: checkOutImages,
...
// POST /api/colleges/:id/clear-duplicate-days - Remove empty unassigned duplicate schedules
router.post('/:id/clear-duplicate-days', authenticate, isSPOCAdmin, async (req, res) => {
    try {
        const { Schedule, Attendance } = require('../models');
        const collegeId = req.params.id;

        const allSchedules = await Schedule.find({ collegeId });
        if (!allSchedules.length) {
            return res.json({ success: true, message: 'No schedules found', deletedCount: 0 });
        }

        const scheduleIds = allSchedules.map((s) => s._id);
        const attendanceRows = await Attendance.find({ scheduleId: { $in: scheduleIds } }).select('scheduleId');
        const scheduleIdsWithAttendance = new Set(attendanceRows.map((a) => String(a.scheduleId)));
        ...
        for (const schedule of allSchedules) {
            ...
            const hasAtt = scheduleIdsWithAttendance.has(sId);
            const hasTrainer = Boolean(schedule.trainerId);
            const hasDate = Boolean(schedule.scheduledDate || schedule.date);

            // Empty unscheduled placeholder card
            if (!hasAtt && !hasTrainer && !hasDate) {
                schedulesToDelete.push(schedule._id);
                continue;
            }

            // Check duplicate slot key: deptId-dayNumber-session-date
            const deptKey = String(schedule.departmentId || 'general');
            const sessionKey = schedule.session || 'FULL_DAY';
            const dateKey = schedule.scheduledDate ? new Date(schedule.scheduledDate).toISOString().split('T')[0] : `day-${schedule.dayNumber}`;
            const slotKey = `${deptKey}_${schedule.dayNumber}_${sessionKey}_${dateKey}`;

            if (seenSlots.has(slotKey) && !hasAtt && !hasTrainer) {
                schedulesToDelete.push(schedule._id);
            } else {
        ...
        if (schedulesToDelete.length) {
            const result = await Schedule.deleteMany({ _id: { $in: schedulesToDelete } });
            deletedCount = result.deletedCount || schedulesToDelete.length;
        }

        res.json({
            success: true,
            message: `Cleared ${deletedCount} duplicate or unscheduled day record(s)`,
            deletedCount,
        });
    } catch (error) {
        console.error('Error clearing duplicate days:', error);
        res.status(500).json({ success: false, message: 'Failed to clear duplicate days', error: error.message });
    }
});
...
router.get('/', authenticate, isSPOCAdmin, async (req, res) => {
    try {
        ...
        } else if (role === 'companyadmin' || role === 'company' || role === 'accountnt' || role === 'collegeadmin') {
    ...
    try {
        ...
        if (courseId && mongoose.Types.ObjectId.isValid(courseId)) {
            ...
            if (courseDoc) {
                ...
                const isLinked = courseDoc.colleges.some(id => String(id) === String(college._id));
                if (!isLinked) {
...
router.post('/:id/upload-attendance', authenticate, isSPOCAdmin, (req, res, next) => {
    ...
    try {
        ...
        college.studentAttendanceExcelUrl = req.file.filename; 
