...
const buildPublicPortalCoreBundle = () => ({
  dashboard: {},
  resources: {},
});
...
const buildSuperAdminDashboardResponse = async () => {
  ...
  const salaryDue = 0;
  const recentActivity = recentAttendance.map((attendance) => {
...
const ensureTrainerProfile = async (userId) => {
  ...
  if (!trainer) {
    // Trainer.email is required+unique: seed it from the User account so
    // auto-provisioning works for users created via the new auth signup.
    const account = await User.findById(userId)
      .select("name firstName lastName email city phoneNumber profile")
      .lean();
    const fullName = account?.name || account?.profile?.fullName || "";
    const [firstName, ...restName] = fullName.split(" ").filter(Boolean);

    const trainerEmail = String(account?.email || "").trim().toLowerCase();
    if (!trainerEmail) {
      throw new Error("Cannot provision trainer profile without a linked account email.");
    }

    const createdTrainer = await Trainer.create({
      userId,
      email: trainerEmail,
      firstName: account?.firstName || firstName || undefined,
      lastName: account?.lastName || (restName.length ? restName.join(" ") : undefined),
      mobile: account?.phoneNumber || account?.profile?.phone || undefined,
  ...
  try {
    ...
    const fetchPayload = async () => {
      ...
      if (bundleScope === "core") {
        ...
        } else if (normalizedRole === "Student" || normalizedRole === "Company") {
          bundle = buildPublicPortalCoreBundle();
        } else {
    ...
    if (forceRefresh) {
      ...
      if (isAvailable()) {
        const ttlSec = Math.ceil(DASHBOARD_BUNDLE_CACHE_TTL_MS / 1000);
        await redis.set(cacheKey, JSON.stringify(responsePayload), 'EX', ttlSec).catch(() => {});
      }
      return res.json({
        success: true,
    ...
    const responsePayload = await withCache(cacheKey, Math.ceil(DASHBOARD_BUNDLE_CACHE_TTL_MS / 1000), fetchPayload);
