...
router.post('/upload', authenticate, authorize(['SuperAdmin', 'SPOCAdmin']), (req, res, next) => {
    ...
    try {
        ...
        for (const row of rows) {
            ...
            let phone = getVal(row, 'Phone', 'Mobile', 'Cell', 'PhoneNumber', 'Contact No', 'Student Phone');
...
router.delete('/clean/:collegeId', authenticate, authorize(['SuperAdmin']), async (req, res) => {
    try {
        ...
        await College.findByIdAndUpdate(collegeId, { $unset: { studentAttendanceExcelUrl: 1 } });
