...
router.get('/today-monitoring', async (req, res) => {
    try {
        ...
        .populate('trainerId', 'firstName lastName email mobile')
        ...
        const trainerDisplayName = (trainer) => {
            ...
            const full = [trainer.firstName, trainer.lastName].filter(Boolean).join(' ').trim();
            return full || trainer.name || trainer.email || 'Unassigned';
        };
        ...
        const monitoringData = schedules.map(schedule => {
            ...
            return {
                id: schedule._id,
                trainer: {
                    name: trainerDisplayName(schedule.trainerId),
                    phone: schedule.trainerId?.mobile || schedule.trainerId?.phone || 'N/A'
                },
                college: {
