...
const STUDENT_ACTIVITY_FILE_FIELDS = new Set([
    'studentsPhoto',
    'activityPhotos',
    'activityVideos',
    'studentActivity',
...
const resolveDriveSubFolderType = (fieldName) => {
    ...
    if (STUDENT_ACTIVITY_FILE_FIELDS.has(fieldName) || f.includes('activity') || f.includes('studentsphoto') || f.includes('student_photo')) {
        return 'studentActivity';
    }
    if (f.includes('checkout') || f.includes('check_out') || f.includes('clock_out')) {
        return 'checkOut';
    }
    if (f.includes('checkin') || f.includes('check_in') || f.includes('clock_in') || f.includes('selfie')) {
        return 'checkIn';
    }
    if (f.includes('attendance') || f.includes('sheet') || f.includes('roster') || f.includes('excel') || f.includes('pdf')) {
        return 'attendance';
    }
    if (GEO_TAG_FILE_FIELDS.has(fieldName)) return 'checkIn';
    return 'attendance';
};
...
const resolveScheduleSessionName = (scheduleDoc = {}, attendance = {}) => {
    const rawSession = scheduleDoc?.sessionType || attendance?.sessionType || attendance?.session;
    if (rawSession && String(rawSession).toUpperCase().includes('AN')) return 'AN';
    if (rawSession && String(rawSession).toUpperCase().includes('FN')) return 'FN';

    const timeStr = scheduleDoc?.startTime || attendance?.checkInTime;
    if (timeStr) {
        const hourMatch = String(timeStr).match(/(\d{1,2}):(\d{2})\s*(AM|PM)?/i);
        if (hourMatch) {
            let hour = parseInt(hourMatch[1], 10);
            const isPM = hourMatch[3] && hourMatch[3].toUpperCase() === 'PM';
            if (isPM && hour < 12) hour += 12;
            if (!isPM && hourMatch[3] && hourMatch[3].toUpperCase() === 'AM' && hour === 12) hour = 0;
            if (hour >= 13) return 'AN';
            return 'FN';
        }
    }
    return 'FN';
...
const collectAttendanceFilesForDriveSync = (attendance) => {
    ...
    const checkInPhotoPath =
        ...
        attendance.checkIn?.photo ||
        attendance.checkInGeoImageUrl ||
        attendance.checkInImage ||
        attendance.check_in_image;
    if (checkInPhotoPath) {
        pushFile('checkInPhoto', checkInPhotoPath);
    }
    pushFile('signature', attendance.signatureUrl);

    // Student Attendance documents, excels, pdfs, and proof photos
    pushFile('attendancePdf', attendance.attendancePdfUrl || attendance.studentAttendancePdfUrl || attendance.scannedAttendancePdfUrl, { mimetype: 'application/pdf' });
    if (attendance.attendancePhoto || attendance.attendance_photo || attendance.attendanceFile || attendance.attendanceDocument) {
        pushFile('attendancePhoto', attendance.attendancePhoto || attendance.attendance_photo || attendance.attendanceFile || attendance.attendanceDocument);
    } else if (attendance.imageUrl && attendance.imageUrl !== checkInPhotoPath) {
        pushFile('attendancePhoto', attendance.imageUrl);
    }

    if (Array.isArray(attendance.studentsPhotoUrl)) {
        pushMany('studentsPhoto', attendance.studentsPhotoUrl);
    } else if (attendance.studentsPhotoUrl && attendance.studentsPhotoUrl !== checkInPhotoPath) {
    ...
    // Check-Out photos and signatures
...
const resolveCanonicalUploadFolders = async ({
    ...
    if (scheduleDoc?.attendanceFolderId && scheduleDoc?.geoTagFolderId) {
        const attendanceFolder = {
            id: scheduleDoc.attendanceFolderId,
            name: scheduleDoc.attendanceFolderName || "Student Attendance",
            webViewLink: scheduleDoc.attendanceFolderLink || null,
        };
        const geoTagFolder = {
            id: scheduleDoc.geoTagFolderId,
            name: scheduleDoc.geoTagFolderName || "Check-In",
            webViewLink: scheduleDoc.geoTagFolderLink || null,
        };
        return {
            checkInFolder: geoTagFolder,
            attendanceFolder,
            studentActivityFolder: attendanceFolder,
            checkOutFolder: geoTagFolder,
            geoTagFolder,
        };
    }

    let attendanceFolder = null;
    ...
    let targetParentId = dayFolderId;

    if (!attendanceFolder) {
        attendanceFolder = await ensureDriveFolderLoader({
            folderName: DRIVE_DAY_SUBFOLDERS.attendance || "Attendance",
            parentFolderId: targetParentId
        });
    }

    if (!geoTagFolder) {
        geoTagFolder = await ensureDriveFolderLoader({
            folderName: DRIVE_DAY_SUBFOLDERS.geoTag || "GeoTag",
            parentFolderId: targetParentId
        });
    }
...
const syncAttendanceFilesToDrive = async ({
    ...
    try {
        ...
        const { attendanceFolder, geoTagFolder, checkInFolder, checkOutFolder, studentActivityFolder } = resolvedFolders;
        const foldersByType = {
            checkIn: toFolderAssetPayload(checkInFolder || geoTagFolder, "Check-In"),
            attendance: toFolderAssetPayload(attendanceFolder, "Student Attendance"),
            studentActivity: toFolderAssetPayload(studentActivityFolder || attendanceFolder, "Student Activities"),
            checkOut: toFolderAssetPayload(checkOutFolder || geoTagFolder, "Check-Out"),
            geoTag: toFolderAssetPayload(geoTagFolder || checkInFolder, "Check-In")
        };
        ...
        const syncResult = await uploadAttendanceFilesToDrive({
            filesByField,
            getTargetFolder: (fieldName) => {
                const subType = resolveDriveSubFolderType(fieldName);
                return foldersByType[subType] || foldersByType.checkIn || foldersByType.attendance || { id: dayFolderId, name: 'Day Folder' };
            },
...
const checkInHandler = async (req, res) => {
    ...
    try {
        ...
        let { trainerId, collegeId, courseId, scheduleId, dayNumber, checkInTime, latitude, longitude, studentsPresent, studentsAbsent } = req.body;
        ...
        // Get file paths as web URLs
        let attendancePdfUrl = null;
        if (req.files?.attendancePdf && req.files.attendancePdf[0]) {
            attendancePdfUrl = `/uploads/attendance/pdfs/${req.files.attendancePdf[0].filename}`;
        }
        let attendanceExcelUrl = null;
        if (req.files?.attendanceExcel && req.files.attendanceExcel[0]) {
            attendanceExcelUrl = `/uploads/attendance/excels/${req.files.attendanceExcel[0].filename}`;
        }
        ...
        const checkInImageFile = req.files?.check_in_image ? req.files.check_in_image[0] : (req.files?.photo ? req.files.photo[0] : (req.files?.image ? req.files.image[0] : null));
        ...
        if (attendance) {
            ...
            if (checkInLocation) {
                attendance.checkIn = {
                    ...
                    photo: checkInImageUrl || attendance.checkIn?.photo || null,
                    signatureUrl: signatureUrl || attendance.checkIn?.signatureUrl || null
                };
            ...
            if (checkInImageUrl) {
                ...
                attendance.checkInPhoto = checkInImageUrl;
...
const enrichAttendanceRecordsWithDocuments = async (attendance = []) => {
    if (!Array.isArray(attendance) || attendance.length === 0) {
        return attendance;
    }

    const attendanceIds = attendance.map((a) => a?._id).filter(Boolean);
    const scheduleIds = attendance.map((a) => a?.scheduleId?._id || a?.scheduleId).filter(Boolean);
    const trainerIds = attendance.map((a) => a?.trainerId?._id || a?.trainerId).filter(Boolean);

    let scheduleDocs = [];
    let checkInDocs = [];
    try {
        const promises = [];
        if (attendanceIds.length > 0 || scheduleIds.length > 0) {
            promises.push(
                ScheduleDocument.find({
                    $or: [
        ...
        if (scheduleIds.length > 0 || trainerIds.length > 0) {
            promises.push(
                CheckIn.find({
                    $or: [
                        { scheduleId: { $in: scheduleIds } },
        ...
        const [docs, cIns] = await Promise.all(promises);
        scheduleDocs = docs || [];
        checkInDocs = cIns || [];
    } catch (e) {
    ...
    return attendance.map((item) => {
        ...
        const matchingDocs = scheduleDocs.filter(
            (d) => (d.attendanceId && String(d.attendanceId) === itemId) ||
                   (d.scheduleId && itemScheduleId && String(d.scheduleId) === itemScheduleId)
        );
        ...
        let attendanceExcelUrl = item.attendanceExcelUrl || item.studentAttendanceExcelUrl || item.attendanceSheetUrl;
        let checkInPhoto = item.checkInPhoto || item.checkIn?.photo || item.checkInGeoImageUrl || item.checkInImage || item.check_in_image;
        let signatureUrl = item.signatureUrl || item.signature || item.checkIn?.signatureUrl;
        let checkInLocation = item.checkInLocation || item.checkIn?.location || (matchingCheckIn?.location ? {
            lat: matchingCheckIn.location.latitude,
            lng: matchingCheckIn.location.longitude,
            address: matchingCheckIn.location.address
        } : null);

        if (!checkInPhoto && matchingCheckIn?.photoUrl) {
            checkInPhoto = matchingCheckIn.photoUrl;
        }
...
router.get('/', async (req, res) => {
    try {
        ...
        if (requestedView === 'geo-verification') {
            ...
            const enrichedAttendance = await enrichAttendanceRecordsWithDocuments(attendance);
        ...
        // Find assigned schedules matching date/filters that don't have an attendance record yet
        const existingScheduleIds = new Set(
            allAttendance.map(a => String(a.scheduleId?._id || a.scheduleId || '')).filter(Boolean)
        );

        let unrecordedSchedules = [];
        if (!verificationStatus && !geoVerificationStatus && !checkOutVerificationStatus) {
            const scheduleFilters = {
                trainerId: { $ne: null },
            };
            if (existingScheduleIds.size > 0) {
                scheduleFilters._id = { $nin: Array.from(existingScheduleIds).map(id => new mongoose.Types.ObjectId(id)) };
            }
            if (startDate || endDate) {
                scheduleFilters.$or = [
            ...
            try {
                ...
                console.warn('Failed to query unrecorded schedules for attendance report:', schErr.message);
            }
        }
        ...
        const pageRecords = shouldPaginate
            ? combinedRecords.slice((page - 1) * limit, page * limit)
            : combinedRecords;

        const enrichedAttendance = await enrichAttendanceRecordsWithDocuments(pageRecords);
...
// PUT /attendance/:id/status - Update Attendance Status (Present / Absent / Late / Leave)
const handleAttendanceStatusUpdate = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const allowedStatuses = ['Present', 'Absent', 'Leave', 'Late', 'Pending', 'cancelled'];
        ...
        if (mongoose.Types.ObjectId.isValid(id)) {
            ...
            if (!attendance) {
                attendance = await Attendance.findOne({ scheduleId: id });
            }
            if (!attendance) {
                ...
                if (schedule) {
                    attendance = new Attendance({
                        ...
                        attendanceStatus: matchedStatus.toLowerCase() === 'present' ? 'PRESENT' : 'ABSENT',
                    });
                }
            }
        }
        ...
        attendance.status = matchedStatus;
        if (matchedStatus.toLowerCase() === 'present') {
            attendance.attendanceStatus = 'PRESENT';
        } else if (matchedStatus.toLowerCase() === 'absent') {
        ...
        if (attendance.scheduleId) {
            try {
                ...
                console.warn('Sync schedule day state warning on status update:', syncErr.message);
            }
        }
        ...
        if (attendance.trainerId) {
            try {
                ...
                console.warn('Cache invalidation warning on status update:', cacheErr.message);
            }
        }

        return res.json({
            success: true,
            message: `Attendance status updated to ${matchedStatus}`,
            data: attendance
        });
    } catch (error) {
        console.error('Error updating attendance status:', error);
        return res.status(500).json({ success: false, message: 'Failed to update attendance status', error: error.message });
    }
};

router.put('/:id/status', authenticate, handleAttendanceStatusUpdate);
router.patch('/:id/status', authenticate, handleAttendanceStatusUpdate);
