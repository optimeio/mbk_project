...
router.get('/pending', authenticate, async (req, res) => {
    try {
        ...
        const pendingUsers = await User.find({
            role: { $in: ['Trainer', 'CompanyAdmin'] },
            accountStatus: 'pending'
        }).select('name email role createdAt accountStatus profilePicture');
...
router.post('/', authenticate, async (req, res) => {
    try {
        ...
        if (requesterRole === 'SuperAdmin') {
            ...
            if (!['SPOCAdmin', 'Trainer', 'SuperAdmin', 'Accountnt', 'Company'].includes(role)) {
                return res.status(403).json({
                    success: false,
                    message: 'Super Admin can only create valid roles (SPOC, Trainer, Accountnt, Company, SuperAdmin).'
                });
            }
        ...
        // Check if email already exists - use global uniqueness validation
        const { email: normalizedEmail } = await validateUniqueness({ 
            email, 
            phone: null // Phone is not provided in admin user creation
        });
...
router.put('/:id/approve', authenticate, async (req, res) => {
    try {
        ...
        if (!companyId) {
            const smCompany = await Company.findOne({ name: { $regex: /sm groups/i } });
            if (smCompany) {
        ...
        // If user is a CompanyAdmin or Company, activate corresponding Company record
        if (user.role === 'CompanyAdmin' || user.role === 'Company') {
            const companyToApprove = await Company.findOne({
                $or: [{ adminId: user._id }, { userId: user._id }, { email: user.email }],
            });
            if (companyToApprove) {
                companyToApprove.status = 'active';
                companyToApprove.isVerified = true;
                companyToApprove.isActive = true;
                await companyToApprove.save();
                console.log(`[INFO] Activated Company record for approved user: ${user.email}`);
            }
        }
        ...
        if (user.role === 'Trainer') {
            ...
            if (!existingTrainer) {
                ...
                existingTrainer.registrationStatus = 'approved';
                existingTrainer.documentStatus = 'approved';
                existingTrainer.status = 'APPROVED';
                if (companyId) existingTrainer.companyId = companyId;
                if (!existingTrainer.email && user.email) {
                    existingTrainer.email = user.email;
                }
        ...
        try {
            ...
            } else if (user.role === 'CompanyAdmin' || user.role === 'Company') {
                const { sendCompanyApprovalEmail } = require('../utils/emailService');
                const loginUrl = `${(process.env.FRONTEND_URL || 'http://localhost:3000').replace(/\/+$/, '')}/login?type=company`;
                await sendCompanyApprovalEmail(user.email, user.name, loginUrl);
            }
        } catch (notifyError) {
            console.error('Failed to notify user of approval:', notifyError);
        }
...
router.put('/notification-preferences', authenticate, async (req, res) => {
    try {
        ...
        user.notificationPreferences = req.body.preferences || req.body;
        await user.save();
