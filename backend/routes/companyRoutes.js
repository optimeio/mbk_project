...
router.put('/:code', authenticate, upload.single('logo'), async (req, res) => {
    try {
        ...
        // SuperAdmin and SPOCAdmin can update any company. Otherwise check admin email match.
        const isSuperAdminOrSpoc = ['SuperAdmin', 'SPOCAdmin'].includes(req.user?.role);
        if (!isSuperAdminOrSpoc) {
            let adminUser = null;
            if (company.adminId) {
