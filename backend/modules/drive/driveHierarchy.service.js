  ...
  Object.entries(dayFoldersByDayNumber || {})
    .map(([dayKey, folderMeta]) => {
      ...
      return {
        ...
        studentActivityFolderId: folderMeta?.studentActivityFolder?.id || null,
        studentActivityFolderName: folderMeta?.studentActivityFolder?.name || null,
        studentActivityFolderLink: folderMeta?.studentActivityFolder?.link || null,
      };
    })
    .filter(Boolean)
...
const ensureDayFolderWithSubFolders = async ({ parentFolderId, dayNumber }) => {
  ...
  const checkInFolder = await ensureDriveFolder({
    folderName: "Check-In",
  ...
  const studentActivityFolder = await ensureDriveFolder({
    folderName: "Student Activities",
    parentFolderId: dayFolder.id,
  });

  const checkOutFolder = await ensureDriveFolder({
  ...
  return {
    ...
    checkInFolder: toFolderPayload(checkInFolder),
    attendanceFolder: toFolderPayload(attendanceFolder),
    studentActivityFolder: toFolderPayload(studentActivityFolder),
    checkOutFolder: toFolderPayload(checkOutFolder),
    geoTagFolder: toFolderPayload(checkInFolder),
  };
};

const isTrainingDriveEnabled = () => true;

const ensureTrainingRootFolder = async () => {
  const rootFolderId =
    EXPLICIT_TRAINING_ROOT_FOLDER_ID ||
    TRAINING_PARENT_FOLDER_ID ||
    getDefaultTrainerDocumentsFolderId() ||
