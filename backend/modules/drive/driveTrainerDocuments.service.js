...
const ensureTrainerCollegeHierarchy = async ({
  ...
  // Create Day 1 to Day N folders, each with Check-In, Student Attendance, Student Activities, and Check-Out subfolders
  ...
  for (let dayNumber = 1; dayNumber <= safeDays; dayNumber += 1) {
    ...
    // FN Session Subfolders
    const fnFolder = await ensureDriveFolder({
      folderName: "FN",
      parentFolderId: dayFolder.id,
    });
    const fnCheckInFolder = await ensureDriveFolder({
      folderName: "Check-In",
      parentFolderId: fnFolder.id,
    });
    const fnAttendanceFolder = await ensureDriveFolder({
      folderName: "Student Attendance",
      parentFolderId: fnFolder.id,
    });
    const fnStudentActivitiesFolder = await ensureDriveFolder({
      folderName: "Student Activities",
      parentFolderId: fnFolder.id,
    });
    const fnCheckOutFolder = await ensureDriveFolder({
      folderName: "Check-Out",
      parentFolderId: fnFolder.id,
    });

    // AN Session Subfolders
    const anFolder = await ensureDriveFolder({
      folderName: "AN",
      parentFolderId: dayFolder.id,
    });
    const anCheckInFolder = await ensureDriveFolder({
      folderName: "Check-In",
      parentFolderId: anFolder.id,
    });
    const anAttendanceFolder = await ensureDriveFolder({
      folderName: "Student Attendance",
      parentFolderId: anFolder.id,
    });
    const anStudentActivitiesFolder = await ensureDriveFolder({
      folderName: "Student Activities",
      parentFolderId: anFolder.id,
    });
    const anCheckOutFolder = await ensureDriveFolder({
      folderName: "Check-Out",
      parentFolderId: anFolder.id,
    });
    ...
    dayFoldersByDayNumber[dayNumber] = {
      ...
      fnFolder: {
        ...toFolderPayload(fnFolder),
        checkInFolder: toFolderPayload(fnCheckInFolder),
        attendanceFolder: toFolderPayload(fnAttendanceFolder),
        studentActivitiesFolder: toFolderPayload(fnStudentActivitiesFolder),
        checkOutFolder: toFolderPayload(fnCheckOutFolder),
      },
      anFolder: {
        ...toFolderPayload(anFolder),
        checkInFolder: toFolderPayload(anCheckInFolder),
        attendanceFolder: toFolderPayload(anAttendanceFolder),
        studentActivitiesFolder: toFolderPayload(anStudentActivitiesFolder),
        checkOutFolder: toFolderPayload(anCheckOutFolder),
      },
      checkInFolder: toFolderPayload(fnCheckInFolder),
      attendanceFolder: toFolderPayload(fnAttendanceFolder),
      studentActivitiesFolder: toFolderPayload(fnStudentActivitiesFolder),
      checkOutFolder: toFolderPayload(fnCheckOutFolder),
      geoTagFolder: toFolderPayload(fnCheckInFolder),
    };
  }
