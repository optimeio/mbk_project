...
const respondWithBootstrapPayload = async ({
  req,
  res,
  loadPayload,
  failureStage,
  ...
  try {
    ...
    logChatControllerError(req, failureStage, error);
    const { buildChatDisabledPayload, CHAT_DISABLED_MESSAGE } = require("../../services/streamChatService");
    return res.status(200).json({
      success: true,
      ...buildChatDisabledPayload(null, error?.message || CHAT_DISABLED_MESSAGE),
    });
  }
};
...
const createChatBootstrapController = ({
  ...
  respondWithBootstrapPayload({
    req,
    res,
    loadPayload: getChatBootstrapPayload,
    failureStage: "get_chat_bootstrap_failed",
  });
...
const createChatQuickBootstrapController = ({
  ...
  respondWithBootstrapPayload({
    req,
    res,
    loadPayload: getChatQuickBootstrapPayload,
    failureStage: "get_chat_quick_bootstrap_failed",
  });
...
const createChatFullBootstrapController = ({
  ...
  respondWithBootstrapPayload({
    req,
    res,
    loadPayload: getChatFullBootstrapPayload,
    failureStage: "get_chat_full_bootstrap_failed",
  });
