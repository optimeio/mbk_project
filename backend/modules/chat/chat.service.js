...
const toBootstrapSuccess = (bootstrap, user = null) => ({
  success: true,
  ...bootstrap,
  bootstrap,
  user: bootstrap?.currentUser || user || null,
  token: bootstrap?.token ?? null,
});

const buildBootstrapDisabledFeed = (message, user = null) =>
  toBootstrapSuccess(buildChatDisabledPayload(user, message), user);

const resolveBootstrapUser = async ({
  currentUserId,
  currentUserRole,
  ...
  if (!currentUserId) {
    return {
      user: null,
      disabledMessage: "Authentication required.",
    };
  }

  const user = await findUserByIdLoader({
    userId: currentUserId,
    roleHint: currentUserRole,
  });

  if (!user) {
    return {
      user: null,
      disabledMessage: CHAT_BOOTSTRAP_USER_NOT_FOUND_MESSAGE,
    };
  }

  return { user, disabledMessage: null };
};

const runBootstrapLoader = async ({
  currentUserId,
  currentUserRole,
  findUserByIdLoader,
  createWorkspaceLoader,
} = {}) => {
  const { user, disabledMessage } = await resolveBootstrapUser({
    currentUserId,
    currentUserRole,
    findUserByIdLoader,
  });
  ...
  if (!user) {
    return buildBootstrapDisabledFeed(disabledMessage);
  }

  try {
    const bootstrap = await createWorkspaceLoader({ user });
    return toBootstrapSuccess(bootstrap, bootstrap?.currentUser || user);
  } catch (error) {
    return buildBootstrapDisabledFeed(
      error?.message || CHAT_DISABLED_MESSAGE,
      user,
    );
  }
};
...
const getChatBootstrapFeed = async ({
  ...
  runBootstrapLoader({
    ...
    currentUserRole,
    findUserByIdLoader,
    createWorkspaceLoader: createWorkspaceBootstrapLoader,
  });
...
const getChatQuickBootstrapFeed = async ({
  ...
  runBootstrapLoader({
    ...
    currentUserRole,
    findUserByIdLoader,
    createWorkspaceLoader: createWorkspaceQuickBootstrapLoader,
  });
...
const getChatFullBootstrapFeed = async ({
  ...
  runBootstrapLoader({
    ...
    currentUserRole,
    findUserByIdLoader,
    createWorkspaceLoader: createWorkspaceFullBootstrapLoader,
  });
