...
const uploadTrainerDocumentFeed = async ({
  ...
  try {
    ...
    fixedDriveFileName = buildFixedTrainerDocumentFileNameLoader(
      normalizedDocType,
      resolvedMimeType,
      file.originalname,
    );

    driveUpload = await uploadToDriveLoader({
...
const getTrainerReviewGate = (trainer = {}) => {
  const hasAgreementSubmission = Boolean(
    (trainer.agreementAccepted ?? trainer.agreementAccepted) && trainer.signature,
  );
