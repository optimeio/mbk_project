...
const listLiveDashboardFeed = async ({
  ...
  if (query?.date) {
    const targetDateStr = dayjs(query.date).format("YYYY-MM-DD");
    startRange = dayjs(targetDateStr).startOf("day").toDate();
    endRange = dayjs(targetDateStr).endOf("day").toDate();
  } else {
    // Timezone-resilient range covering current UTC and local timezone shifts (e.g. IST UTC+5:30)
    startRange = dayjs().subtract(12, "hour").startOf("day").toDate();
    endRange = dayjs().add(12, "hour").endOf("day").toDate();
  }

  const filter = {
    scheduledDate: { $gte: startRange, $lte: endRange },
    status: { $ne: "cancelled" },
  };
  ...
  if (isSpocRole(user?.role)) {
    ...
    const spocOrs = [];
    if (scopedCompanyIds.length > 0) {
      spocOrs.push({ companyId: { $in: scopedCompanyIds } });
    }
    if (scopedCompanyCodes.length > 0) {
      spocOrs.push({ companyCode: { $in: scopedCompanyCodes } });
    }
    if (userId) {
      spocOrs.push({ createdBy: userId });
    }

    if (spocOrs.length > 0) {
      filter.$and = [{ $or: dateOrFilter }, { $or: spocOrs }];
      delete filter.$or;
    } else {
      return { success: true, count: 0, data: [] };
    }
...
const hasAttendanceDocs = (attendance) =>
  Boolean(
    ...
    || attendance?.studentAttendancePdfUrl
    || attendance?.studentAttendanceExcelUrl
    || attendance?.scannedAttendancePdfUrl
    || attendance?.attendanceSheetUrl
    || (Array.isArray(attendance?.documents) && attendance.documents.some((d) =>
      /attendance/i.test(String(d?.type || d?.documentType || d?.name || ''))
    ))
    || (Array.isArray(attendance?.scheduleDocuments) && attendance.scheduleDocuments.some((d) =>
      /attendance/i.test(String(d?.type || d?.documentType || d?.name || ''))
    )),
  );
...
const hasGeoTagDocs = (attendance) =>
  Boolean(
    ...
    || (Array.isArray(attendance?.photos) && attendance.photos.length)
    || (Array.isArray(attendance?.images) && attendance.images.length)
    || (Array.isArray(attendance?.documents) && attendance.documents.some((d) =>
      /geotag|photo|selfie|activity/i.test(String(d?.type || d?.documentType || d?.name || ''))
    ))
    || (Array.isArray(attendance?.scheduleDocuments) && attendance.scheduleDocuments.some((d) =>
      /geotag|photo|selfie|activity/i.test(String(d?.type || d?.documentType || d?.name || ''))
    )),
...
const buildDayUploadStatus = (schedule, attendance) => {
  ...
  if (hasTrainerAssigned && isPast && !hasCheckIn && !isAttendancePending && normalizedScheduleStatus !== "cancelled") {
      return {
          ...
          geoTagUploaded,
          statusCode: "absent",
          statusLabel: "Absent / No Check-In",
      };
  }
...
const buildTrainerSchedulesPayload = ({
  ...
  attendanceDocs.forEach((attendance) => {
    ...
    const currentByKey = latestAttendanceByKey.get(key);
    if (isPreferredAttendanceCandidate(attendance, currentByKey)) {
      latestAttendanceByKey.set(key, attendance);
    }

    const currentById = latestAttendanceByScheduleId.get(scheduleId);
    if (isPreferredAttendanceCandidate(attendance, currentById)) {
      latestAttendanceByScheduleId.set(scheduleId, attendance);
    }
  });
  ...
  return schedules.map((schedule) => {
    ...
    const attendance =
      latestAttendanceByKey.get(key) ||
      latestAttendanceByScheduleId.get(originalId) ||
      latestAttendanceByScheduleId.get(scheduleId) ||
      null;
    const dayUploadStatus = buildDayUploadStatus(schedule, attendance);
...
const withTimeout = (promise, ms = 2000, fallbackValue = null) => {
  let timer;
...
const resolveTrainerNmTrainersFolderFields = async ({
  ...
  if (!mongoose.Types.ObjectId.isValid(trainerId)) return {};
  ...
  try {
    ...
    schedulesAsyncLogger.warn({
      ...
      reason: driveErr?.message || "Unknown error",
  ...
  if (!collegeEntry) {
    ...
    collegeEntry.active = true;
    collegeEntry.status = "active";
  }
  ...
  } else if (!trainerDoc.collegeId || String(trainerDoc.collegeId) !== String(college._id)) {
    trainerDoc.collegeId = college._id;
  }
  ...
  try {
    ...
    console.warn("[TrainerSave] Warning:", saveErr?.message);
  }

  // Always sync TrainerAssignment model so getActiveAssignment and Trainer Portal find this assignment
  try {
    const { syncTrainerAssignmentRecord, resolveTrainerDisplayName } = require("../../utils/trainerAssignmentResolver");
    await syncTrainerAssignmentRecord({
      trainer: trainerDoc,
      trainerName: resolveTrainerDisplayName(trainerDoc),
      collegeName: college.name,
      driveFolderId: hierarchy?.collegeFolder?.id || trainerDoc.collegeDriveFolderId,
    });
  } catch (assignSyncErr) {
    console.warn("[TrainerAssignmentSync] Warning:", assignSyncErr?.message);
  }
...
const validateTrainerApproved = async (trainerId) => {
  if (!trainerId) return;
  const mongoose = require("mongoose");
  if (!mongoose.Types.ObjectId.isValid(trainerId)) return;
  const { Trainer } = require("../../models");
  const trainerDoc = await Trainer.findById(trainerId).populate("userId", "isActive name");
  if (!trainerDoc) {
    const error = new Error("Selected trainer not found");
...
const checkTrainerScheduleConflict = async ({ trainerId, scheduledDate, startTime, endTime, session, excludeScheduleId = null }) => {
  ...
  try {
    ...
    console.warn("checkTrainerScheduleConflict check failed:", err.message);
  }
        ...
        try {
          if (typeof sendInAppNotificationLoader === "function") {
            await sendInAppNotificationLoader(io, {
              ...
              message: `Training Assigned - ${courseForNotification?.title || "TEST COURSE"} (${dayNumber ? `Day ${dayNumber}` : "Day 1"}). ${collegeForNotification?.name} on ${dayjs(scheduledDate).format("DD-MM-YYYY")} (${startTime} - ${endTime}). Contact SPOC: ${spocName} (${spocPhone})`,
              type: "Schedule",
...
const assignScheduleFeed = async ({
  ...
  const { trainerId, scheduledDate, startTime, endTime, session } = payload;
      ...
      try {
        if (typeof sendInAppNotificationLoader === "function") {
          await sendInAppNotificationLoader(io, {
            ...
            role: "Trainer",
            title: "Training Assigned",
            message: `Training Assigned - ${course?.title || "TEST COURSE"} (${schedule.dayNumber ? `Day ${schedule.dayNumber}` : "Day 1"}). ${college?.name} on ${dayjs(scheduledDate).format("DD-MM-YYYY")} (${startTime || schedule.startTime} - ${endTime || schedule.endTime}). Contact SPOC: ${spocName} (${spocPhone})`,
            type: "Schedule",
            link: "/trainer/schedule",
          });
        }
...
const updateScheduleFeed = async ({
  ...
  if (payload?.trainerId !== undefined && payload.trainerId) {
    await validateTrainerApproved(payload.trainerId);
  }
        ...
        try {
          if (typeof sendInAppNotificationLoader === "function") {
            await sendInAppNotificationLoader(io, {
              ...
              message: `Training Rescheduled - ${course?.title || "TEST COURSE"} (${updatedSchedule.dayNumber ? `Day ${updatedSchedule.dayNumber}` : "Day 1"}). New Date: ${newDateFormatted} (${updatedSchedule.startTime} - ${updatedSchedule.endTime}). Contact SPOC: ${spocName} (${spocPhone})`,
              type: "Schedule",
              link: "/trainer/schedule",
            });
          }
        ...
        try {
          if (typeof sendInAppNotificationLoader === "function") {
            await sendInAppNotificationLoader(io, {
              ...
              message: `Training Cancelled - ${course?.title || "TEST COURSE"}. ${college?.name} on ${dayjs(schedule.scheduledDate).format("DD-MM-YYYY")}. Reason: ${reason}. Contact SPOC: ${spocName} (${spocPhone})`,
              type: "Schedule",
              link: "/trainer/schedule",
            });
          }
...
const mapAssociationsPayload = ({
  ...
  companies: companiesRaw.map((company) => {
    ...
    if (company.companyCode !== undefined) entry.companyCode = company.companyCode;
    return entry;
  }),
...
const listScheduleAssociationsFeed = async ({
  ...
  listTrainers = listAssociationsTrainers,
  listDepartments = listAssociationsDepartments,
  insertDepartments = insertAssociationsDepartments,
} = {}) => {
  const [companiesRaw, coursesRaw, collegesRaw, trainersRaw] = await Promise.all([
    listCompanies(),
    listCourses(),
    listColleges(),
    listTrainers(),
