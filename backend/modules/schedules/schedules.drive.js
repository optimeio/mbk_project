  ...
  async ({
    ...
    try {
      ...
      if (department?._id) {
        ...
        const departmentHierarchy = await withTimeout(
          ensureDepartmentHierarchyLoader({
            company: companyRef,
            course: courseRef,
            college,
            department,
      ...
      } else if (college?._id && normalizedDayNumber) {
        let companyRef = company;
        if (!companyRef?._id && college.companyId) {
          companyRef = await loadCompanyById({ companyId: college.companyId });
        }
        if (!companyRef?._id) {
          const anyCompany = await Company.findOne({ isArchived: { $ne: true } }).lean();
          companyRef = anyCompany || { _id: college.companyId || college._id, name: "MBK Technologies" };
        }

        const collegeHierarchy = await withTimeout(
          ensureCollegeHierarchyLoader({
            company: companyRef,
            course: course || null,
            college,
          }),
