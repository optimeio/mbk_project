...
import { FILE_BASE_URL, api } from '@/services/api';
...
const DayDetailsModal = ({ open, onClose, day, college, trainers = [], onVerify, onSave, onDelete }) => {
    ...
    useEffect(() => {
        if (open && (!trainers || trainers.length === 0)) {
            api.get('/trainers')
                .then((res) => {
                    const list = Array.isArray(res) ? res : (res?.trainers || res?.data || []);
                    setFetchedTrainers(list);
                })
                .catch(() => {});
        }
    }, [open, trainers]);
                                        ...
                                        <div className="mt-3 text-center sm:mt-0 sm:text-left w-full">
                                            ...
                                            {isEditing ? (
                                                ...
                                                <div className="mt-6 space-y-6">
                                                    ...
                                                    {(() => {
                                                        const targetScheduleId = day.id || day._id || day.scheduleId || '';
                                                        const attendanceUrl = targetScheduleId ? `/dashboard/attendance?scheduleId=${targetScheduleId}` : '/dashboard/attendance';
                                                        return (
                                                            <div className="grid md:grid-cols-2 gap-4 mb-6">
                                                                <div 
                                                                    onClick={() => router.push(attendanceUrl)}
                                                                    className="bg-green-50 border border-green-200 rounded-xl p-5 cursor-pointer hover:bg-green-100 transition-all hover:shadow-md group"
                                                                >
                                                                ...
                                                                <div 
                                                                    onClick={() => router.push(attendanceUrl)}
                                                                    className="bg-red-50 border border-red-200 rounded-xl p-5 cursor-pointer hover:bg-red-100 transition-all hover:shadow-md group"
                                                                >
                                                                    <div className="flex items-center justify-between">
                                                                        <p className="text-xs text-red-700 uppercase font-semibold">
                                                                            Students Absent
                                                                        </p>
                                                                        <span className="text-[10px] text-red-600 font-medium opacity-0 group-hover:opacity-100 transition-opacity">
                                                                            View details &rarr;
                                                                        </span>
                                                                    </div>
                                                    ...
                                                    <div className="space-y-4">
                                                        ...
                                                        {(() => {
                                                            const evidenceItems = [];
                                                            const checkInUrl = day.checkInPhoto || day.checkInImage || day.checkIn?.photo || (day.imageUrl && String(day.imageUrl).toLowerCase().includes('check_in') ? day.imageUrl : null) || day.checkInGeoImageUrl;
                                                            if (checkInUrl) {
                                                                evidenceItems.push({
                                                                    url: checkInUrl,
                                                            ...
                                                            const checkOutImages = Array.isArray(day.checkOutGeoImageUrls) && day.checkOutGeoImageUrls.length
                                                                ? day.checkOutGeoImageUrls
                                                                : (day.checkOutGeoImageUrl ? [day.checkOutGeoImageUrl] : []);
                                                            checkOutImages.forEach((url, idx) => {
                                                                if (url && !evidenceItems.some(item => item.url === url)) {
                                                                    evidenceItems.push({
                                                                        url,
                                                                        label: `Check-Out Photo ${idx + 1}`,
                                                                        type: "image"
                                                                    });
                                                                }
                                                            });
                                                            if (Array.isArray(day.activityPhotos)) {
                                                                day.activityPhotos.forEach((url, idx) => {
                                                                    if (url && !evidenceItems.some(item => item.url === url)) {
                                                                        evidenceItems.push({
                                                                            url,
                                                                            label: `Activity Photo ${idx + 1}`,
                                                                            type: "image"
                                                                        });
                                                                    }
                                                                });
                                                            }
                                                            if (day.signatureUrl && !evidenceItems.some(item => item.url === day.signatureUrl)) {
                                                                evidenceItems.push({
                                                                    url: day.signatureUrl,
                                                                    label: "Trainer Signature",
                                                                    type: "image"
                                                                });
                                                            }
                                                            if (Array.isArray(day.activityVideos)) {
                                                                day.activityVideos.forEach((url, idx) => {
                                                                    if (url) {
                                                                        evidenceItems.push({
                                                                            url,
                                                                            label: `Activity Video ${idx + 1}`,
                                                                            type: "video"
                                                                        });
                                                                    }
                                                                });
                                                            ...
                                                            return (
                                                                <div className="rounded-xl border border-gray-200 overflow-hidden">
                                                                    <div className="bg-gray-50 px-4 py-2 border-b border-gray-200 flex justify-between items-center">
                                                                        <h4 className="text-xs font-medium text-gray-500 uppercase tracking-wider">Photos & Evidence ({evidenceItems.length})</h4>
                                                                    </div>
                                                                    <div className="p-4">
                                                                        <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
                                                                            {evidenceItems.map((item, index) => (
                                                                                <div key={`evidence-${index}`} className="relative flex-none w-48 group">
                                                                                    {item.type === "image" ? (
                                                                                        <>
                                                                                            <img loading="lazy"
                                                                                                src={toImageSrcUrl(item.url)}
                                                                                                alt={item.label}
                                                                                            ...
                                                                                            <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 transition-all rounded-lg flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
                                                                                                ...
                                                                                                <a
                                                                                                    href={toViewUrl(item.url, { download: true })}
                                                                                                    download
                                                                                                    target="_blank"
                                                                                                    rel="noopener noreferrer"
                                                                                                    className="text-white bg-black bg-opacity-50 rounded-full p-2 hover:bg-opacity-75"
                                                                                        ...
                                                                                        <div className="h-32 w-full bg-gray-100 rounded-lg border border-gray-200 flex items-center justify-center relative">
                                                                                            ...
                                                                                            <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 transition-all rounded-lg flex items-center justify-center opacity-0 group-hover:opacity-100">
                                                                                                <a
                                                                                                    href={item.url.startsWith('http') ? item.url : `${FILE_BASE_URL}/api/${item.url.replace(/\\/g, '/').replace(/^\/+/, '')}?token=${localStorage.getItem('accessToken')}`}
                                                                                                    target="_blank"
                                                                                                    rel="noopener noreferrer"
                                                                                                    className="text-white bg-black bg-opacity-50 rounded-full p-2 hover:bg-opacity-75"
                                                                                                >
                                                                                    ...
                                                                                    <p className="mt-1 text-xs text-center text-gray-500 font-medium truncate">{item.label}</p>
                                                                                </div>
                                                                            ))}
                                                                        </div>
                                                                    </div>
