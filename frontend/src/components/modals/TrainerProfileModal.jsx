  ...
  useEffect(() => {
    ...
    const fetchSchedules = async () => {
      try {
        ...
        const trainerId = encodeURIComponent(String(trainerRecordId));
        const response = await api.get(`/schedules/trainer/${trainerId}`);
        const list = Array.isArray(response)
          ? response
          : response?.data || response?.schedules || [];
        if (!isDisposed) {
          setTrainerSchedules(list);
        }
      } catch (err) {
            ...
            >
              <Dialog.Panel className="dashboard-modal-panel relative w-full max-w-2xl transform rounded-3xl border border-gray-100 bg-white text-left shadow-2xl transition-all">
                ...
                <div className="px-6 py-8 sm:px-10">
                  ...
                  {/* Scheduled Classes & Assignments Section */}
                  <div className="mt-6 border-t border-slate-200 pt-6">
                    <div className="mb-3 flex items-center justify-between">
                      <h4 className="flex items-center text-sm font-black uppercase tracking-wider text-slate-800">
                        <CalendarDaysIcon className="mr-2 h-4 w-4 text-indigo-600" />
                        Assigned Schedules ({trainerSchedules.length})
                      </h4>
                    </div>
                    ...
                    {schedulesLoading ? (
                      ...
                      <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/50 p-4 text-center text-xs font-semibold text-slate-500">
                        No upcoming or assigned schedules found for this trainer.
                      </div>
                    ) : (
                      <div className="max-h-60 overflow-y-auto space-y-2 pr-1">
                        {trainerSchedules.map((sched, idx) => {
                          const collegeName =
                            sched.collegeId?.name ||
                            sched.collegeName ||
                            sched.college ||
                            "College N/A";
                          const courseTitle =
                            sched.courseId?.title ||
                            sched.courseName ||
                            sched.topic ||
                            sched.subject ||
                            "N/A";
                          const schedDate =
                            sched.scheduledDate
                              ...
                              : sched.startDate
                              ? `${new Date(sched.startDate).toLocaleDateString()} - ${new Date(sched.endDate).toLocaleDateString()}`
                              : "Date N/A";
                          let schedDay = "Day 1";
                          if (sched.dayNumber) {
                            if (sched.scheduledDate) {
                              const d = new Date(sched.scheduledDate);
                              if (!isNaN(d.getTime())) {
                                const weekday = d.toLocaleDateString("en-US", { weekday: "short" });
                                schedDay = `Day ${sched.dayNumber} (${weekday})`;
                              } else {
                                schedDay = `Day ${sched.dayNumber}`;
                              }
                            } else {
                              schedDay = `Day ${sched.dayNumber}`;
                            }
                          } else if (sched.day) {
                            schedDay = String(sched.day);
                          } else if (sched.dayOfWeek) {
                            schedDay = String(sched.dayOfWeek);
                          } else if (Array.isArray(sched.days) && sched.days.length > 0) {
                            schedDay = sched.days.join(", ");
                          } else {
                            const dateVal = sched.scheduledDate || sched.startDate || sched.date;
                            if (dateVal) {
                              const d = new Date(dateVal);
                              if (!isNaN(d.getTime())) {
                                schedDay = d.toLocaleDateString("en-US", { weekday: "long" });
                              }
                            }
                          }
                          const schedTime =
                            sched.startTime && sched.endTime
                              ? `${sched.startTime} - ${sched.endTime}`
                              : sched.timeSlot || sched.slot || sched.time || "Time N/A";

                          return (
                            <div
                              key={sched._id || idx}
                              className="rounded-2xl border border-slate-200 bg-white p-3.5 shadow-xs transition hover:border-indigo-200"
                            >
                              <div className="flex flex-wrap items-center justify-between gap-2">
                                <span className="font-bold text-slate-800 text-xs flex items-center gap-1">
                                  🏫 {collegeName}
                                </span>
                                <span className="inline-flex items-center rounded-full bg-indigo-50 px-2.5 py-0.5 text-[10px] font-black text-indigo-700 uppercase">
                                  📚 {courseTitle}
                                </span>
                              </div>
                              <div className="mt-2 grid grid-cols-3 gap-2 text-[11px] text-slate-600 font-medium border-t border-slate-100 pt-2">
                                <div>
                                  <span className="text-slate-400 block text-[9px] font-black uppercase">Date</span>
                                  📅 {schedDate}
                                </div>
                                <div>
                                  <span className="text-slate-400 block text-[9px] font-black uppercase">Day</span>
                                  🗓️ {schedDay}
                                </div>
                                <div>
                                  <span className="text-slate-400 block text-[9px] font-black uppercase">Time</span>
                                  ⏰ {schedTime}
                                </div>
                              </div>
                            </div>
