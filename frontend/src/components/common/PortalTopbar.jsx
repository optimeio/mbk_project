...
function PortalTopbarInner({ onMenuClick = () => {} }) {
  ...
  return (
    <header className="relative z-40 shrink-0 overflow-hidden border-b border-slate-200/80 bg-white/90 backdrop-blur-xl">
      <div
        className="flex min-w-0 items-center gap-2 px-4 sm:gap-3 sm:px-6 h-16 justify-between sm:h-20"
      >
        <div className="min-w-0">
          <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.22em] text-[#0f3f5c]">
            <span className="inline-flex h-2 w-2 rounded-full bg-[#1d5f87]" />
            <span className="truncate">{portalTitle}</span>
          </div>
          <h1 className="mt-0.5 truncate text-lg font-bold text-slate-900 sm:text-xl md:text-2xl">
            {sectionTitle}
          </h1>
        </div>
