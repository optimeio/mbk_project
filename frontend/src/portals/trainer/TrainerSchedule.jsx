...
const TrainerSchedule = ({ initialSelectedMonth }) => {
    ...
    const [currentTrainerId, setCurrentTrainerId] = useState(() => currentUser?.trainerId || currentUser?.trainerProfileId || currentUser?.trainer?._id || '');
    ...
    // Load trainer profile ID to fetch the trainer's schedules
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                const response = await getTrainerProfile();
                const trainerObj = response?.data || response?.trainer || response;
                const nextTrainerId = resolveEntityId(trainerObj?._id || trainerObj?.id || trainerObj);
                if (nextTrainerId) {
