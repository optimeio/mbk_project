...
const CityDetails = () => {
    ...
    const cityName = useMemo(() => {
        if (routeId && routeId !== '1') return decodeURIComponent(routeId);
        if (typeof window === 'undefined') return routeId;
        // Extract city name from URL path, e.g. /dashboard/cities/Chennai
        const parts = window.location.pathname.split('/').filter(Boolean);
        const cityIdx = parts.indexOf('cities');
        if (cityIdx !== -1 && parts[cityIdx + 1]) return decodeURIComponent(parts[cityIdx + 1]);
        return routeId;
    }, [routeId]);
