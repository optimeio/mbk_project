...
const CompanyDetails = ({ companyId: propCompanyId } = {}) => {
    ...
    const getCourseImageUrl = (imageValue) => {
        ...
        const prefix = FILE_BASE_URL ? FILE_BASE_URL : '/api';
        ...
        if (normalizedPath.includes('/uploads/')) {
            ...
            return `${prefix}${uploadsPath}${authQuery}`;
        }

        // Stored as filename only.
        const fileName = normalizedPath.split('/').pop();
        return `${prefix}/uploads/trainer-documents/${encodeURIComponent(fileName)}${authQuery}`;
    };
    ...
    const companyDetailsQuery = useQuery({
        ...
        queryFn: async () => {
            try {
                const [companyRes, coursesRes, collegesRes, trainersRes] = await Promise.all([
                    api.get(`/companies/${id}`).catch(() => null),
                    api.get(`/courses?companyId=${id}`).catch(() => []),
                    api.get(`/colleges?companyId=${id}`).catch(() => []),
                    api.get(`/trainers`).catch(() => [])
                ]);

                const coursesData = Array.isArray(coursesRes) ? coursesRes : (coursesRes?.data || []);
                const allColleges = Array.isArray(collegesRes) ? collegesRes : (collegesRes?.data || []);
                const allTrainers = Array.isArray(trainersRes) ? trainersRes : (trainersRes?.data?.trainers || trainersRes?.data || []);

                const companyTrainers = allTrainers.filter((t) => {
                    const cId = t.companyId?._id || t.companyId;
                    const cCode = t.companyCode;
                    return (cId && String(cId) === String(id)) || (cCode && companyRes?.companyCode && String(cCode).toUpperCase() === String(companyRes.companyCode).toUpperCase());
                });

                const collegesByCourse = new Map();
                allColleges.forEach((college) => {
                    const courseIdKey = String(college?.courseId?._id || college?.courseId || '');
                    if (!courseIdKey) return;
                    if (!collegesByCourse.has(courseIdKey)) {
                        collegesByCourse.set(courseIdKey, []);
                    }
                    collegesByCourse.get(courseIdKey).push(college);
                });

                const coursesWithColleges = coursesData.map((course) => ({
                    ...course,
                    colleges: collegesByCourse.get(String(course._id)) || [],
                }));

                return {
                    company: companyRes || null,
                    courses: coursesWithColleges,
                    trainers: companyTrainers,
                };
            } catch (err) {
                console.error("Error fetching company details:", err);
                return { company: null, courses: [], trainers: [] };
            }
        }
    });
    ...
    const companyTrainers = companyDetailsQuery.data?.trainers || [];
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="relative h-44 bg-gradient-to-br from-indigo-50 to-blue-50 flex items-center justify-center overflow-hidden">
                                            ...
                                            <AcademicCapIcon className="h-16 w-16 text-indigo-200 absolute" />
                                            {/* Upload Image Button */}
                                            <label
                ...
                {/* Company Trainers Section */}
                <div className="mt-8 bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                            <UserIcon className="h-6 w-6 text-indigo-600" />
                            <h2 className="text-xl font-bold text-gray-900">Company Trainers</h2>
                            <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-indigo-100 text-indigo-800">
                                {companyTrainers.length}
                            </span>
                        </div>
                    </div>
                    ...
                    {companyTrainers.length === 0 ? (
                        <div className="p-8 text-center text-gray-400 text-sm font-medium">
                            No trainers assigned to this company yet.
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            {companyTrainers.map((tr) => {
                                const name = tr.name || [tr.firstName, tr.lastName].filter(Boolean).join(' ') || tr.userId?.name || 'Trainer';
                                const email = tr.email || tr.userId?.email || 'N/A';
                                const phone = tr.mobile || tr.phone || tr.userId?.phoneNumber || 'N/A';
                                const city = tr.city || tr.userId?.city || 'N/A';

                                return (
                                    <div key={tr._id || tr.id} className="p-4 rounded-xl border border-slate-100 bg-slate-50/50 flex items-start gap-3">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <p className="text-xs text-gray-400 mt-1">{city} • {phone}</p>
                                        </div>
                                    </div>
                                );
                            })}
