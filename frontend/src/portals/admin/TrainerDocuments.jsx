...
const TrainerDocuments = () => {
  ...
  const fetchTrainers = async () => {
    try {
      ...
      if (selectedTrainerId) {
        const targetLower = String(selectedTrainerId).toLowerCase();
        const matchedTrainer = documentWorkspace.find((trainer) => {
          const trainerDocId = String(trainer.id || trainer._id || "").toLowerCase();
          const userId = String(
            typeof trainer.userId === "object"
              ? trainer.userId?._id || trainer.userId?.id || ""
              : trainer.userId || ""
          ).toLowerCase();
          ...
          return (
            trainerDocId === targetLower ||
            userId === targetLower ||
            email === targetLower ||
            (urlSearch && (email.includes(targetLower) || name.includes(targetLower)))
          );
        });
  ...
  useEffect(() => {
    ...
    const syncRequestedTrainerId = () => {
      ...
      const nextTrainerId = searchParams.get("trainerId") || "";
      const nextSearch = searchParams.get("search") || "";
      setRequestedTrainerId(nextTrainerId || nextSearch);
      if (nextSearch) {
  ...
  const canApproveProfile = Boolean(selectedTrainer);
