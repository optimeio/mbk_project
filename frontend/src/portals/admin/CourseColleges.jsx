...
const CollegeStats = ({ courseId, collegeId }) => {
  const { data: stats, isLoading } = useQuery({
    ...
    queryFn: () => api.get(`/batches/stats?courseId=${courseId}&collegeId=${collegeId}`).catch(() => ({ totalBatches: 0, totalTrainers: 0, totalStudents: 0 })),
    staleTime: 60 * 1000,
  });
...
const CourseColleges = () => {
  ...
  const companyId = useMemo(() => {
    ...
    const queryId = searchParams.get('companyId') || searchParams.get('id');
  ...
  const courseId = useMemo(() => {
    ...
    if (typeof window === 'undefined') return routeId;
    const searchParams = new URLSearchParams(window.location.search);
    const queryId = searchParams.get('courseId');
    if (queryId) return queryId;
    const matches = window.location.pathname.match(/[a-f\d]{24}/ig) || [];
      ...
      try {
        if (isCourseFiltered) {
          const [companyRes, coursesRes, collegesRes] = await Promise.all([
            api.get(`/companies/${companyId}`).catch(() => null),
            api.get(`/courses?companyId=${companyId}`).catch(() => []),
            api.get(`/colleges?courseId=${courseId}`).catch(() => []),
          ]);
        ...
        return { company: null, course: null, colleges: [], companies: [], courses: [] };
      }
    },
    ...
    if (isCourseFiltered) {
      return [
        { label: 'Companies', value: 'Companies', to: '/dashboard/companies' },
        {
          ...
          to: companyId ? `/dashboard/companies?id=${companyId}` : '/dashboard/companies',
        },
        {
  ...
  const handleConfirmDelete = async (password) => {
    ...
    try {
      ...
      if (deleteItem.type === 'course') {
        ...
        setDeleteItem(null);
        router.push(companyId ? `/dashboard/companies?id=${companyId}` : '/dashboard/companies');
        return;
      }
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex justify-between items-center">
            <button
              onClick={() => router.push(companyId ? `/dashboard/companies?id=${companyId}` : '/dashboard/companies')}
              className="flex items-center text-gray-600 hover:text-gray-900 transition-colors"
            >
              <ArrowLeftIcon className="h-5 w-5 mr-2" />
              {companyId ? 'Back to Company' : 'Back to Companies'}
            </button>
            <button
