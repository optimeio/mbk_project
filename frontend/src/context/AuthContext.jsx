...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const handleUnauthorized = (event) => {
      const currentToken = typeof window !== 'undefined'
        ? (localStorage.getItem('accessToken') || localStorage.getItem('authToken') || localStorage.getItem('token'))
        : null;
      if (currentToken) {
        // Active token exists; avoid kicking out user on sporadic background endpoint failures
        return;
      }
  ...
  const login = async (email, password, options = {}) => {
    ...
    const throwLoginProviderError = (response, fallback = 'Invalid email or password') => {
      ...
      err.status = response?.status || (isRateLimited ? 429 : 0);
      if (isRateLimited) {
