...
const MainLayout = ({ children }) => {
    ...
    // Keep latest currentUser and setAuthUser in a stable ref so the polling
    // interval function can read them without being re-created on every render.
    const currentUserRef = useRef(currentUser);
    const setAuthUserRef = useRef(setAuthUser);
    useEffect(() => { currentUserRef.current = currentUser; }, [currentUser]);
    useEffect(() => { setAuthUserRef.current = setAuthUser; }, [setAuthUser]);

    // Refresh trainer verification status on mount and periodically.
    // Uses a ref-stable callback so the interval is only created ONCE on mount,
    // eliminating the previous bug where `currentUser` as a dependency caused the
    // interval to be torn down and recreated on every render (causing extra API calls).
    const refreshTrainerStatus = useCallback(async () => {
        const user = currentUserRef.current;
        if (normalizeAuthRole(user?.role, user?.email) !== AUTH_ROLES.TRAINER) return;
        try {
            const response = await api.get('/trainers/profile/me', { skipCache: true });
            if (response?.success && response?.data) {
                setAuthUserRef.current({ ...user, ...response.data });
            }
        } catch (error) {
    ...
    useEffect(() => {
        // Only poll for trainers (check from stable initial role)
        if (normalizeAuthRole(currentUserRef.current?.role, currentUserRef.current?.email) !== AUTH_ROLES.TRAINER) {
            return;
        }
        // Run once on mount, then every 5 minutes (profile data changes infrequently)
        refreshTrainerStatus();
        const interval = setInterval(refreshTrainerStatus, 5 * 60 * 1000);
        return () => clearInterval(interval);
    }, [refreshTrainerStatus]); // refreshTrainerStatus is stable (no deps)
