...
const handleResponse = async (response, context = {}) => {
  ...
  if (!response.ok) {
    ...
    if (body && typeof body === "object") {
      const candidate = body.message || body.error || body.msg || body.data;
    ...
    const endpointLabel = context?.endpoint || context?.url || "";
    const methodLabel = (context?.method || "GET").toUpperCase();
    if (response.status >= 500) {
      console.warn(`[API ${response.status}] ${methodLabel} ${endpointLabel}:`, errorMessage, body);
    }
    ...
    error.response = body;
    error.endpoint = context?.endpoint || null;
...
const fetchWithAuth = async (endpoint, options = {}) => {
  ...
  const requestPromise = (async () => {
    ...
    const result = await handleResponse(response, { endpoint, url, method });
    recordApiLatencyMetric({
      method,
      endpoint,
      status: response?.status || 0,
...
export const forgotPassword = async (email) => {
  ...
  return handleResponse(response, { endpoint: "/auth/forgot-password", method: "POST" });
};
...
export const verifyResetOTP = async (email, otp) => {
  ...
  return handleResponse(response, { endpoint: "/auth/verify-reset-otp", method: "POST" });
};
...
export const resetPassword = async (tempToken, password) => {
  ...
  return handleResponse(response, { endpoint: "/auth/reset-password", method: "POST" });
};
