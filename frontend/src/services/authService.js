...
export const registerCompany = (data) =>
  api.post("/simple-auth/company/register", data);

export const sendCompanyOtp = (data) =>
  api.post("/simple-auth/company/send-otp", data);
...
export const companyAuthService = {
  sendOtp: async (email) => {
    return await makeRequest('/company/send-otp', 'POST', { email: String(email || '').trim().toLowerCase() });
  },
  register: async (formData) => {
...
export const authService = {
  sendCompanyOtp: (data) => sendCompanyOtp(data),
  logout: async () => {
